...
def draw_heat_map(data, i):
    ...
    plt.savefig("figures/"+str(i)+"-BJ.png")
...
def calculate_flows(data):
    # Calculate flow values from inflow/outflow data
    # Input shape: (D, T, H, W, 2) where last dim is [inflow, outflow]
...
def process_data(data, pred=False):
    # Process and split traffic flow data
    D = 416  # Number of days
    T = 24   # Hours per day
...
def get_dataloader(datapath, start_day, end_day, nb_ts_per_day, batch_size, grid_size):
    ...
    with open('data/MALMCS_data/MALMCS_data/pred_all_stresnet_mf4_masked.pkl', 'rb') as f:
...
def get_mask(frames):
    ...
    study_area_polygon = loads('POLYGON ((116.4 39.8, 116.4 39.8, 116.4 39.8, 116.4 39.8))')
    x, y = study_area_polygon.exterior.xy
    study_area_transformed = []
    for i in range(len(x)):
        study_area_transformed.append(wgs84_to_gcj02(x[i], y[i]))
    study_area_transformed_polygon = Polygon(study_area_transformed)
    _, H, W = frames.shape
    mask = generate_region_mask(H, W, study_area_transformed_polygon)
    return mask
...
def generate_region_mask(H, W, polygon):
    ...
    for i in range(H):
        for j in range(W):
            lat, lng = revert_transform(j, i, -53, 26, 39.8, 116.4)
            pt = Point(lng, lat)
