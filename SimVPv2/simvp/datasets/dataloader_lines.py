...
"""
The dataset for Lines is expected to be organized as follows:
    <data_root>/lines/ 
    ├── train/
    │   ├── 100/ 
    │   │   ├── 0.png
...
def load_data(batch_size, val_batch_size, data_root, num_workers=4, pre_seq_length=5, aft_seq_length=1):
    ...
    train_set_path = os.path.join(data_root, "lines/train")
    test_set_path = os.path.join(data_root, "lines/test")

    transform_video = transforms.Compose([transforms.Resize(
        size=(128, 128), interpolation=Image.NEAREST), transforms.ToTensor(),])

    train_set = Lines(video_root=train_set_path, video_ext='png',
                            nframes=6, loader=video_loader, transform=transform_video)
    test_set = Lines(video_root=test_set_path, video_ext='png',
                           nframes=6, loader=video_loader, transform=transform_video)
