...
def get_config():
    ...
    # Data configuration
    parser.add_argument("--data_path", type=str, default="data/MALMCS_data/MALMCS_data/", help="data path")
    ...
    # Agent configuration
    parser.add_argument("--init_pos", type=list, default=[32,66], help="initial position of agents")
    parser.add_argument("--energy", type=int, default=40, help="agent energy limit")
    parser.add_argument("--grid_size", type=list, default=[51,108], help="size of grid world")
...
def main():
    ...
    # Initialize model
    model = Model(args.input_size, args.hidden_size, args.output_size, args.agent_num, args.init_pos, 
                 args.grid_size, args.energy, args.capacity, device).to(device)
    print(model)
    ...
    for epoch in range(args.epochs):
        ...
        for (heat_map, pred_heat_map) in train_loader:
            ...
            if iter % args.save_iter == 0:
                ...
                message = "Epoch [{}/{}] ({}) Train loss: {:.4f}, Val loss: {:.4f}, Coverage: {:.2f}".format(
                    epoch, args.epochs, iter, np.mean(epoch_train_losses), val_loss, val_coverage
                )
                print(message)
                model.train()
